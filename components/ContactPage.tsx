  ...
  return (
    <div className="fixed inset-0 z-[300] overflow-y-auto">
      <ThemedBackground className="min-h-screen flex flex-col">
        <button onClick={onClose} className="fixed top-5 right-5 z-20 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors text-lg font-bold backdrop-blur-sm">×</button>
        {sent ? (
          ...
          <div className="flex-1 flex flex-col lg:flex-row">
            <div className="lg:w-1/2 flex flex-col items-center justify-center bg-black/30 backdrop-blur-sm border-r border-white/5 px-10 py-16 gap-6">
              ...
              <div className="flex flex-col items-center gap-2 mt-2">
                ...
                <a href="mailto:hello@topcheesehockey.com" className="text-cyan-400 hover:text-cyan-300 font-bold text-sm transition-colors">hello@topcheesehockey.com</a>
              </div>
            </div>
            <div className="lg:w-1/2 flex flex-col justify-center px-8 sm:px-12 py-16">
              <div className="max-w-md bg-black/30 backdrop-blur-sm border border-white/5 rounded-2xl p-8">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="John Smith" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-cyan-500/40 transition-colors" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Email Address</label>
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-cyan-500/40 transition-colors" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Message</label>
                    <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="How can we help you?" rows={6} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-cyan-500/40 transition-colors resize-none" />
                  </div>
                  <button onClick={handleSubmit} disabled={sending} className={`w-full py-4 rounded-xl font-bold text-sm transition-all ${sending ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg'}`}>
                    {sending ? (
