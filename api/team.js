...
// Stripe subscription — same lookup check-subscription.js does,
...
// ── List a team's members — was GET /api/team-members?ownerEmail=... ──
...
// ── Invite a member — was POST /api/team-invite ─────────────────────
...
// ── Remove a member — was POST /api/team-remove ─────────────────────
...
// ── Single entry point — routed by HTTP method, and by an "action"
// field in the body for the two POST cases, since invite and remove
// were previously two separate POST endpoints.
export default async function handler(req, res) {
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error('Team management error: SUPABASE_SERVICE_ROLE_KEY is not set in the environment.');
  ...
  if (req.method === 'POST') {
    ...
    return res.status(400).json({ error: 'Missing or invalid "action" — expected "invite" or "remove".' });
  }
